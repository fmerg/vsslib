...
do
    arg="$1"
    case $arg in
        hash|hmac|aes|elgamal|core|shamir|signer|plain|ies|kem|andDlog|eqDlog|ddh|dlog|fiatShamir|linearDlog|okamoto|nizk|crypto|lagrange|keys|backend|arith|bitwise|serializers)
            MODULE="$arg"
            shift
            ;;
        -r|--reload)
